...
date: 2018-03-21 10:00:00 PST
tags: architecture, pwa, state, redux
slug: architecting-uis-for-change
...

## Techniques for keeping your app code under control when requirements shift dramatically
...
Personally, I've observed this in so many apps that I feel confident that it's the source of many of these issues. For context, I've built a wide range of what might be considered "ambitious" web apps in my career. Real-time asset geo-location tracking systems, chat apps, video calling apps, the Starbucks PWA, apps that enable you to make and receive real phone calls in a browser, surgery tracking apps, and recently I've been working on an e-commerce PWA for one of the world's largest retailers. The observations seem to hold true regardless of team size.
...
To some degree, this is inevitable, and not all bad. But, if we look at the above sequence there is a precise moment where things seem to go off the rails.

> > It happens at the point where the original assumptions are challenged.

Can we perhaps define the point more clearly? I think so. As I hinted at above, more often than not, the precise point where "loss of control" over the app begins is when someone realizes the potential benefit of connecting logic from two previously disparate areas of the application. I've started to believe that most of us are drawing the wrong boundaries when we first build an app. I think that happens as a natural consequence of trying to draw boundaries that make sense to us when we're thinking about the data the app will consume. I think this happens for two reasons:

1. Our brains have a bias toward broad generalizations and simple solutions.
2. We don't fully understand what the real requirements should be until we try to build a solution.

But, when we're first building an app, we can relatively easily identify the types of _data_ we'll have to keep track of. The data often fits into neat little buckets. The trouble is:
...
So, from a data perspective we'll have products, categories, and carts and from a process perspective, people will search, they'll add to cart, they'll checkout.

Simple, right?!

So, as developers and product managers we have some really clean, simple lines, right? We'll obviously need an API that returns collections of products as search results, and as items in your cart.

So when splitting up the work of building a UI, we kind of let the data types split the UI concerns too. Part of the UI will be all about searching for products. There's one clean line. Then we'll need a product detail view that pulls in relevant product data like full descriptions, images, and user reviews. Then we'll need a cart view, and a checkout view.
...
Then, a smart UX designer comes along and says: "You know, we really ought to have customers be aware of their cart the whole time. It's like walking around in a store, you have your cart with you and you should be able to metaphorically glance down at it whenever you want without leaving your shopping experience."

This is hard to argue with. Of course that's a better experience. So, now what? You already have a completely separate team managing the "cart" experience. They own everything that happens at the `yourstore.com/cart` page.
...
But soon, when people get a taste for this ability they say "well wouldn't it be awesome if we used our knowledge of what was in the cart to enhance and customize, well... pretty much the whole experience!" It goes on and on:
...
Our previous lines of separation break down. The idea of "the product" we're currently viewing is no longer a standalone concept. Now we really care about the product in the context of the cart and the context of other things we may want to search for, etc. This may seem annoying at first, but should not come as a surprise.
...
> > UIs, by their very nature, are all about connecting disparate types of data! Yet somehow, when building UIs our data structures often fail to account for this.

So what happens? Change slows, and engineers start to push back on what really would have been fairly simple change requests if it had been built differently.

What could we have done? We can't anticipate what specific features we're going to be requested to add.
...
> > Perhaps our mental models are wrong?
...
Option #1 is the embodiment of the shopping cart example I mentioned above. Apps that I've personally built this way (which were quite a few, I first wrote about [adopting backbone in 2010](https://blog.andyet.com/2010/10/29/building-a-single-page-app-with-backbonejs-undersc/)) always got messy when trying to share models between collections, or somehow associate models with one another. The one _really_ strong point in _favor_ of this model/collection approach is it often perfectly mimics the data structures we get from data APIs.

Option #2 works great for simpler stuff. In my option, one of the main reasons the React programming model is so successful is it solved the nested component problem. Before React we were always hacking together "child views" but we always had to manage them ourselves. But the shortcoming with using components for everything is that it doesn't directly address the problem of state that clearly is not directly tied to a single component. With React 16+ we have the new context API. This helps pass state around, but still doesn't directly help us solve the structuring and management of "global app state." From what I've seen, this frequently leads to a giant interconnected mess of components that are hard to to reason about as a whole.

Option #3 has some incredibly cool demos, but has been problematic for many to implement in the real world. As it turns out, dealing with a single app-global state object comes with its own set of challenges. Gaps in functionality of Redux (and similar state tools) have been filled by third party libs and many times, the _core idea_ of having a "god object" is a bit at odds with other popular libraries. For example, redux and React-Router have not always played nicely together. And other gaps in functionality have led to a large crop of supplemental libraries such as redux-saga (or redux-loop), Immutable.js, and the process of connecting redux to something like react has historically been a bit messy. After after writing a ton of `mapDispatchToProps`, `mapStateToProps` suddenly the idea feels a little less glorious.

That summary of the three options is a bit disappointing, do all our models just suck?
...
2. Use a component hierarchy like React, or my favorite, Preact, but let's use it for what it's best at. Which is rendering a tree of visual components. Let's use local component state for things that are truly local to the component and its children. Usually local state is perfect for things transient UI state, such as whether or not a particular line item in a list is in "editing mode." Or tracking form values and form field error states while the user is in the process of filling out a form.
...
From my experience, plucking the best patterns from each is great. But, there are still shortcomings in the "god object" approach. Luckily, I've found that those pitfalls can be avoided. Unlike trying to hold strict boundaries between different data-types or "pages" within an app, there are other boundaries we can draw that are much more defensible because they're not linked to the app's feature set.

They are:

1. Don't introduce _any_ other sources of truth. If you're doing the "god object" thing. By all means, don't have multiple "gods." If you need data from something like a browser API write some glue code that mirrors the relevant state from the browser into your "god object." This includes things like the current URL. ([read more about how I do routing with redux here](https://read.reduxbook.com/markdown/part2/09-routing.html)).
2. _always_ read state through an abstraction. There's a concept called "selectors" that is essentially a function that takes the state from the "god object" and returns just the slice of state you care about for a given thing. By always reading state through selectors you can retain flexibility in how you choose to structure the "raw" data of the "god object". Also, selectors enable you to "subscribe" to a very specific subset of changes within your "god object" enabling efficient rendering.
3. Precisely _never_ store derived state in the "god object." Only store state in its rawest, purest form. You can decorate and enrich it as it's being read through selectors.
4. For extra credit: never use `componentDidMount`, or something similar to initiate data fetching. Because, contrary to popular opinion, whether or not a particular component is mounted has very little to do with whether or not the app actually should consider fetching the data. The shopping cart example above is a good example of that. Regardless of whether the user is looking at their cart or not, we want to know what's in their cart because we can enhance the entire rest of the experience based on that data. Instead, ideally we want to be able to just declare a maximum stale age that we're ok with, and it should refresh the data behind the scenes whenever it's too old. To accomplish this we can use selectors to subscribe to our "god object" and look for conditions which should trigger other changes.
...
5. You can init and "run" an entire functional application _without any visual components whatsoever_, this is analogous to running a browser in headless mode.
...
## Applying these ideas to our shopping cart
...
1. The cart should be fetched and be available no matter where you are in the app.
...
// a selector that always returns a usable number of cart items
// regardless of the state of cart items. Even if we just haven't
// fetched the cart items yet.
selectCartItemCount: createSelector('selectCartItemsRaw', items =>
...
```
// now we change the existing selector to instead sum up
// the `quantity` attribute of our cart items.
selectCartItemCount: createSelector('selectCartItemsRaw', items =>
...
```
selectCartItemIds: createSelector('selectCartItemsRaw', items => {
  if (!items) {
    return null
  }
...
(again, i'm glossing over some of the details, in order to focus on the ideas)
...
reactShouldFetchRelatedProducts: createSelector(
  ...
  (cartItems, recommendedItems) => {
    ...
    const idsHaveChanged = recommendedItems.lastFetchedIdsAsString !== idString
...
Now let's tackle the requirement of modifying the item detail page if the item is already in the cart.

Crap! Now we have another two, seemingly distinct, concerns that all of a sudden care about each other.
...
selectActiveItem: createSelector(
  'selectItemDataById',  // grabs all item data we have cached by ID
...
There are obviously still implementation details I'm completely ignoring here. To fully explain all those things and the tools and libraries I actually use to do all of this in practice I'd have to write a whole book, which I actually did and made available online for free: https://read.reduxbook.com.
